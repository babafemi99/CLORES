...
func Orders(c *fiber.Ctx) error {
	...
	for i, o := range orders {
		orders[i].Name = o.FullName()
		orders[i].Total = o.GetTotal()
	}
...
func CreateOrder(c *fiber.Ctx) error {
	...
	stripe.Key = "sk_test_51JcSPgELI1RUpoSTwvp29u0zdLAzxhdAnJ6LvYk0h4OyNzxDSpmH5XcMGQvkYILaXTiGVYYXpyTdwaYbDZQsipU600ng3oMEBI"
...
func CompleteOrder(c *fiber.Ctx) error {
	...
	go func(order models.Order) {
		...
		ambassadorMessage := []byte(fmt.Sprintf("You earned $%f from the link #%s", adminRevenue, order.Code))
		smtp.SendMail("0.0.0.0:1025", nil, "no-reply@email.com", []string{order.AmbassadorEmail}, ambassadorMessage)

		adminMesssage := []byte(fmt.Sprintf("Order #%d with a total of $%f has been completed", order.Id, adminRevenue))
		smtp.SendMail("0.0.0.0:1025", nil, "no-reply@email.com", []string{"admin@admin.com"}, adminMesssage)
	}(order)
